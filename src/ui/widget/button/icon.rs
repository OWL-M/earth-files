...
//! Vendored from pop-os/libcosmic d9431dc, src/widget/button/icon.rs
...
impl<Message> Button<'_, Message> {
    pub fn new(icon: Icon) -> Self {
        let theme = crate::ui::theme::spacing();
...
impl<'a, Message: Clone + 'static> From<Button<'a, Message>> for Element<'a, Message> {
    fn from(builder: Button<'a, Message>) -> Element<'a, Message> {
        ...
        content.push(
            crate::ui::widget::icon(builder.variant.handle.clone())
        ...
        if !builder.label.is_empty() {
            content.push(
                crate::ui::widget::text(builder.label)
        ...
        let mut button = if builder.variant.vertical {
            iced::widget::Column::with_children(content)
                ...
                .spacing(builder.spacing.to_pixels())
        ...
        if builder.tooltip.is_empty() {
            ...
            tooltip(
                button,
                crate::ui::widget::text(builder.tooltip)
                    .size(builder.font_size.to_pixels())
                    .font(crate::ui::font::Font {
                        weight: builder.font_weight,
                        ..crate::ui::font::default()
