    ...
    for Button<'a, Message>
{
    ...
    fn draw(
        ...
        let mut styling = if !is_enabled {
            ...
            if state.is_pressed {
                if !self.selected && matches!(self.style, crate::ui::theme::Button::HeaderBar) {
        ...
        if matches!(
            self.style,
            crate::ui::theme::Button::MenuItem | crate::ui::theme::Button::MenuFolder
        ) {
            match theme.list_item_position {
                ...
                None => {
                    styling.border_radius = theme.cosmic().radius_0().to_radius();
                }
            }
        }

        let mut icon_color = styling
            .icon_color
        ...
        } = &self.variant
        {
            renderer.with_layer(*viewport, |renderer| {
                ...
                let c_rad = active().cosmic().corner_radii;
            ...
            if on_press.is_some() || on_press_down.is_some() {
                ...
                if cursor.is_over(bounds) {
                    ...
                    if let Some(on_press_down) = on_press_down {
                        let msg = (on_press_down)(iced_core::Vector::ZERO, layout.bounds());
                        shell.publish(msg);
                    }
