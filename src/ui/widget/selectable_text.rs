...
///
/// Wraps the plain [`Text`](crate::ui::widget::Text) widget to add text selection
/// and a context menu (Copy / Select All), both on by default. Created
/// with [`selectable_text`]; the plain [`text`](crate::ui::widget::text)
/// constructor and the typography presets return the unwrapped
/// [`Text`](crate::ui::widget::Text), so non-selectable text keeps iced's looser
/// trait bounds.
///
...
impl<'a> SelectableText<'a> {
    ...
    where
        <crate::ui::Theme as Catalog>::Class<'a>:
            From<crate::ui::widget::text::StyleFn<'a, crate::ui::Theme>>,
    {
    ...
    #[must_use]
    pub fn class(mut self, class: impl Into<<crate::ui::Theme as Catalog>::Class<'a>>) -> Self {
...
fn w<'x, Message>(inner: &'x InnerText<'_>) -> &'x dyn Widget<Message, crate::ui::Theme, Renderer> {
    inner
}

fn w_mut<'x, Message>(
    inner: &'x mut InnerText<'_>,
) -> &'x mut dyn Widget<Message, crate::ui::Theme, Renderer> {
    inner
}
    ...
    for SelectableText<'a>
{
    ...
    fn draw(
        ...
        if state.is_keyboard_focused() {
            ...
            let accent: Color = cosmic.accent.base.to_color();
            renderer.fill_quad(
                renderer::Quad {
    ...
    fn overlay<'b>(
        ...
        if self.shows_context_menu() && !self.uses_popup_context_menu() {
            ...
            crate::ui::widget::text_context_menu::context_menu_overlay(
                &self.inner,
                inner_tree,
                None,
                translation,
...
pub fn selectable_text<'a>(content: impl Into<Cow<'a, str>> + 'a) -> SelectableText<'a> {
    SelectableText::new(content.into()).font(crate::ui::font::default())
}

// Typography presets, mirroring [`crate::ui::widget::text`]
macro_rules! selectable_preset {
    ($(#[$meta:meta])* $name:ident) => {
        $(#[$meta])*
        pub fn $name<'a>(content: impl Into<Cow<'a, str>> + 'a) -> SelectableText<'a> {
            SelectableText::from(crate::ui::widget::text::$name(content))
        }
    };
}
