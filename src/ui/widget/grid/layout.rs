...
//! Vendored from pop-os/libcosmic d9431dc, src/widget/grid/layout.rs
...
pub fn resolve<Message>(
    ...
    // border, so the scroll origin and the border-box origin coincide).
    let grid_size = Size {
        width: grid_layout.scrollable_overflow_rect.right,
        height: grid_layout.scrollable_overflow_rect.bottom,
    };
