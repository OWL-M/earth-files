...
//! Vendored from pop-os/libcosmic d9431dc, src/widget/settings/item.rs
...
impl<'a, Message: Clone + 'static> Item<'a, Message> {
    ...
    fn label(self) -> Element<'a, Message> {
        if let Some(description) = self.description {
            crate::ui::widget::Column::with_capacity(2)
                .spacing(2)
    ...
    pub fn toggler(
        ...
        list::button(
            self.control(
                crate::ui::widget::toggler(is_checked)
                    .width(Length::Shrink)
                    .on_toggle(message),
            ),
        )
        ...
        F: Fn(V) -> Message,
    {
        ...
        list::button(
            self.control_start(crate::ui::widget::radio::Radio::new_no_label(
                value, selected, f,
            )),
        )
        .on_press(on_press)
