...
//! Vendored from pop-os/libcosmic d9431dc, src/widget/flex_row/layout.rs
...
pub fn resolve<Message>(
    ...
    // taffy 0.14 replaced `Layout::content_size` with `Layout::scrollable_overflow_rect`,
    // a rect measured from the node's scroll origin. Its `right`/`bottom` are the same
    // end-extents that `content_size.width`/`.height` used to report (this node has no
    // border, so the scroll origin and the border-box origin coincide) -- with one
    // exception: taffy 0.9's flexbox always added the container's trailing padding to
    // `content_size`, whereas 0.14 only does so for scroll containers (CSS-correct; this
    // node is `overflow: visible`). Add it back so the row keeps reserving its own
    // trailing padding, as it did on 0.9.
    let content_width = flex_layout.scrollable_overflow_rect.right + padding.right;
    let content_height = flex_layout.scrollable_overflow_rect.bottom + padding.bottom;

    let size = Size {
        width: content_width,
        height: actual_height.max(content_height),
    };
